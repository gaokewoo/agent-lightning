...
from agentlightning.algorithm.utils import batch_iter_over_dataset, serialize_for_logging, with_llm_proxy, with_store
from agentlightning.reward import find_final_reward
from agentlightning.types import Dataset, NamedResources, PromptTemplate, RolloutMode, RolloutStatus
...
class APO(Algorithm, Generic[T_task]):
    ...
    def __init__(
        ...
        *,
        gradient_model: str = "deepseek-chat",
        apply_edit_model: str = "deepseek-chat",
    ...
    async def textual_gradient_and_apply_edit(
        ...
        *,
        ...
        """
        # 1) Critique
        critique_text: str | None = await self.compute_textual_gradient(
            current_prompt,
            rollout,
            prefix=prefix,
        )
        ...
        # === PROMPT DEBUG INFO ===
        self._log(logging.INFO, "=" * 80, prefix=prefix)
        self._log(logging.INFO, "1. CURRENT PROMPT (before edit)", prefix=prefix)
        self._log(logging.INFO, "-" * 80, prefix=prefix)
        self._log(logging.INFO, current_prompt.prompt_template.template, prefix=prefix)
        
        self._log(logging.INFO, "=" * 80, prefix=prefix)
        self._log(logging.INFO, "2. AE_MSG (messages sent to LLM)", prefix=prefix)
        self._log(logging.INFO, "-" * 80, prefix=prefix)
        self._log(logging.INFO, str(ae_msg["messages"]), prefix=prefix)  # type: ignore
        
        self._log(logging.INFO, "=" * 80, prefix=prefix)
        self._log(logging.INFO, "3. NEW PROMPT (after edit)", prefix=prefix)
        self._log(logging.INFO, "-" * 80, prefix=prefix)
        self._log(logging.INFO, new_prompt or "<NO NEW PROMPT GENERATED>", prefix=prefix)
        self._log(logging.INFO, "=" * 80, prefix=prefix)
        # === END PROMPT DEBUG INFO ===
    ...
    async def get_rollout_results(
        ...
        *,
        ...
        for r in rollout:
            ...
            self._log(logging.INFO, f"gaoke spans: {json.dumps(serialize_for_logging(spans))}", prefix=prefix)
            self._log(logging.INFO, f"gaoke adapter messages: {json.dumps(serialize_for_logging(messages))}", prefix=prefix)
    ...
    async def evaluate_prompt_on_batch(
        ...
        *,
        ...
        # gaoke: RESOURCES CREATED
        self._log(logging.INFO, f"gaoke resources: {json.dumps(serialize_for_logging(resources))}", prefix=prefix)
        
        resource_update: ResourcesUpdate = await store.update_resources(prompt.version, resources)
        
        # gaoke: RESOURCES UPDATED
        self._log(logging.INFO, f"gaoke resources updated: {json.dumps(serialize_for_logging(resource_update.model_dump()))}", prefix=prefix)

        rollout_ids: List[str] = []
        for t in dataset:
            r: Rollout = await store.enqueue_rollout(input=t, mode=mode, resources_id=resource_update.resources_id)
            # gaoke: ROLLOUT ENQUEUED
            self._log(logging.INFO, f"gaoke rollout enqueued: {json.dumps(serialize_for_logging(r.model_dump()))}", prefix=prefix)
        ...
        # gaoke: ROLLOUT RESULTS
        self._log(logging.INFO, f"gaoke rollout finished: {json.dumps(serialize_for_logging(finished))}", prefix=prefix)

        self._log(logging.INFO, f"gaoke rollout_results: {json.dumps(serialize_for_logging(rollout_results))}", prefix=prefix)
